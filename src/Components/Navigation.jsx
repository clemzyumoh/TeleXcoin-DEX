...
const Navigation = ({darkMode}) => {
  const Menus = [
    ...
    },
    {
      to: "/swap",
      label: "Liquidity",
      icon: <TbCoinTakaFilled className="text-xl" />,
      dis: "translate-x-16",
    },
    {
      to: "/foundation",
            ...
            <li
              ...
              <span
                className={` text-xl cursor-pointer z-10 duration-500  ${
                  index === active &&
                  "-mt-9 dark:text-white bg-white dark:bg-transparent  text-black dark:border-2 rounded-full p-2  shadow-[2px_2px_5px_rgba(0,0,0,0.2),-1px_-1px_5px_rgba(0,0,0,0.2)] dark:shadow-[3px_3px_10px_rgba(0,0,0,0.6),-8px_-8px_10px_rgba(255,255,255,0.1)]"
                }`}>
                {menu.icon}
              </span>
              <span
